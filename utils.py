...
def entropy(sample):
    ...
    probs = {c: sample.count(c) / len(sample) for c in sample}
    e: float = 0  # entropy
...
def format_branches(root):
    ...
    curr_path = []  # (feature, branch_value), used to construct a single path
    format_str = "{}:{}={} "
    branches = []  # paths within the tree
    ...
    def dfs(node):
        ...
        if isinstance(node, Leaf):  # create output entry
            curr_branch = format_str * len(curr_path)
            format_list = [(i + 1, v[0], v[1]) for i, v in enumerate(curr_path)]
            curr_branch = curr_branch.format(*[i for tpl in format_list for i in tpl])  # this is pure magic
...
def format_prediction_params(params):
