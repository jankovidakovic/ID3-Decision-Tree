...
class DecisionTree:
    ...
    def fit(self, data):
        ...
        self.__root = self.__id3(data, data, 0)
        self.__branches = format_branches(self.__root)

        return self

    def __id3(self, dataset, parent_dataset, depth):
        ...
        if self.__max_depth is None or depth < self.__max_depth:  # depth limit not reached
            if len(dataset) == 0:  # empty dataset
                ...
                mdf = dataset.most_discriminatory_feature
                sub_datasets = dataset.group_by_feature(mdf)
                node = Node(mdf, dataset.most_frequent_label)
                # for each child, create the corresponding decision subtree
                for feature_value, sub_dataset in sub_datasets.items():
                    child_node = self.__id3(sub_dataset, dataset, depth + 1)  # create the subtree
    ...
    def predict(self, dataset):
        ...
        prediction_params = {"branches": self.__branches}
        predicted_values = []
        ...
        prediction_params["confusion_matrix"] = \
            ConfusionMatrix(dataset.label_space, dataset.label_sample, predicted_values)

        return prediction_params

    def __label_example(self, example, node):
